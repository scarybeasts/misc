MODE7:HIMEM=&4A00:VDU129,157,131:PRINT"Disc BEAST v0.4.3":PRINT
...
IF A$="INIT" THEN PROCini:UNTIL FALSE
IF A$="DUMP" THEN PROCd(V%(0)):UNTIL FALSE
IF A$="BFIL" THEN PROCbfil:UNTIL FALSE
IF A$="BSET" THEN PROCbset:UNTIL FALSE
IF A$="SEEK" THEN T%=V%(0):PROCs:UNTIL FALSE
IF A$="RIDS" THEN PROCclr:PROCrids:PROCr:PRINT"HEADERS: "+STR$(S%):PROCd(0):UNTIL FALSE
IF A$="READ" THEN PROCclr:PROCread:PROCr:PROCdtim:L%=FNl(Z%)-B%:!C%=-1:PROCc32(B%,L%,C%):PROCc32f(C%):PRINT"CRC32 "+STR$(L%)+" BYTES: "+STR$~(!C%):PROCd(0):UNTIL FALSE
IF A$="RTRK" THEN PROCclr:PROCrtrk:PROCr:PRINT"LEN: "+STR$(S%):PROCd(0):UNTIL FALSE
IF A$="WRIT" THEN PROCrw(24):PROCr:UNTIL FALSE
IF A$="WTRK" THEN PROCwtrk:PROCr:UNTIL FALSE
IF A$="TIME" THEN CALL D%+21:PRINT"DRIVE SPEED: "+STR$(FNl(Z%+4)):UNTIL FALSE
IF A$="DTRK" THEN PROCdtrk:UNTIL FALSE
IF A$="DCRC" THEN PROCdcrc:UNTIL FALSE
IF A$="HFEG" THEN PROChfeg:UNTIL FALSE
IF A$="HFEP" THEN PROChfep:UNTIL FALSE
...
IF A$="BFUN" THEN ?(Z%+10)=V%(0):UNTIL FALSE
IF A$="FSYS" THEN F$=Q$:G%(6)=-1:PRINT"FSYS "+F$:UNTIL FALSE
IF A$="CMFM" THEN PROCcmfm:UNTIL FALSE
IF A$="SCRC" THEN PROCscrc:UNTIL FALSE
IF A$="FCRC" THEN PROCfcrc:UNTIL FALSE
IF A$<>"" THEN PRINT "???"
UNTIL FALSE
...
DEF PROCdtrk
PROCtrk:J%=?(B%+5)
PRINT"TRACK "T%" LEN"FNl(B%+6)
PRINT "CRC32 "+STR$~(!C%)
IF R%=2 THEN PRINT"FAIL"
IF J%=0 THEN ENDPROC
FOR I%=0 TO J%-1
!C%=-1:L%=FNrsiz(I%)+1:M%=FNsaddr(I%):N%=?M%:?M%=N% OR &F0:PROCc32(M%,L%,C%):?M%=N%
PRINT STR$(I%)+": "+STR$~(!C%);
IF FNcrcerr(I%) THEN VDU129:PRINT" !CRC";
IF FNsizem(I%) THEN VDU131:PRINT" !SIZE";
IF FNidtrk(I%)<>T% THEN VDU134:PRINT" !TRACK";
PRINT
NEXT
ENDPROC
...
IF V%(5)=0 OR F$="DRV" THEN ENDPROC
PROCfsel:*SAVE M7SCR 7C00 +400
A%=G%(0):CALL D%+3
ENDPROC
...
IF F$="DRV" THEN PROCdrv:A%=G%(0):CALL D%+3:ENDPROC
IF (T% AND 1)=0 AND T%<>V%(7) THEN ENDPROC
PROCfsel:OSCLI("SAVE TRKS"+STR$(T% AND &FE)+" 5C00 +2000"):A%=G%(0):CALL D%+3
ENDPROC
...
DEF PROCfsel
OSCLI(F$):A%=G%(6):IF V%(7)>40 AND T%>39 THEN A%=A%+2
...
PROCs:PROCbufs(M%+N%,&5400):V%(0)=1
REPEAT:PROCwtrk:IF R%<>0 THEN PROCr
UNTIL R%=0
ENDPROC
